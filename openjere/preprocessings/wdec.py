...
class WDec_preprocessing(ABC_data_preprocessing):
    ...
    def gen_vocab(self, min_freq: int):
        ...
        word_vocab.update(
            {v: i + word_size for i, v in enumerate(relation_vocab.keys())}
        )
        print(len(word_vocab), rel_size, word_size)
        assert len(word_vocab) == rel_size + word_size
    ...
    def _read_line(self, line: str) -> Optional[str]:
        ...
        if "spo_list" in instance:
            ...
            for spo in spo_list:
                spo['object'] = ' '.join(spo['object'])
                spo['subject'] = ' '.join(spo['subject'])
