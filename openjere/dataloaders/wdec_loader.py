...
class WDec_Dataset(Abstract_dataset):
    ...
    def __getitem__(self, index):
        ...
        trg_words, back_trg_words = self.seq2id(seq)    
    ...
    def seq2id(self, seq):
        ...
        def get_trg_back(tuples):
            ...
            trg_words = list(map(lambda x: self.word_vocab.get(x, oov), trg_words))
            return trg_words
        trg_words = get_trg(tuples)
        back_trg_words = get_trg_back(tuples_back)
        return trg_words, back_trg_words

class Batch_reader(object):
    def __init__(self, data):
        data.sort(key=lambda x: len(x[0]), reverse=True)
        transposed_data = list(zip(*data))
        self.tokens_id = torch.tensor(seq_padding(transposed_data[0]))#按批次padding的词id

        # TODO
        self.trg_words = torch.tensor(seq_padding(transposed_data[1]))#按批次padding的解码id
        # back_tri = [[item[0]] + item[1:-1][::-1] + [item[-1]] for item in transposed_data[1]]
        back_tri = transposed_data[-1]
        self.back_trg_words = torch.tensor(seq_padding(back_tri))#按批次padding的解码id
        self.target = torch.tensor(seq_padding([ins[1:] for ins in transposed_data[1]]))#trg_words去掉“[SOS]”开始标记
        self.back_target = torch.tensor(seq_padding([ins[1:] for ins in back_tri]))
        self.src_words_mask = torch.eq(self.tokens_id, 0)#padding的mask
        # self.seq_words_mask = torch.gt(self.seq_id, 0)

        self.en_len = transposed_data[2]#输入的每句话长度   未padding
        self.de_len = transposed_data[3]#解码的输出序列长度   未padding
        self.trg_vocab_mask = torch.tensor(transposed_data[4]).bool()#整个词典去不属于本句话的的mask
        self.spo_gold = transposed_data[5]#spo
        self.text = transposed_data[6]#原文
