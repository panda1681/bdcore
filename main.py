...
class Runner(object):
    def __init__(self, exp_name: str):
        ...
        logging.basicConfig(
            filename=os.path.join("experiments", self.exp_name + "_0524.log"),
            filemode="w",
    ...
    def run(self, mode: str):
        if mode == "preprocessing":
            ...
            f1, log = self.evaluation(loader, generate_wrongcase=True)
            print(log)
            print("f1 = ", f1)
    ...
    def evaluation(self, loader, generate_wrongcase=False):
        ...
        result = self.model.get_metric(generate_wrongcase=generate_wrongcase)
        # print(
        #     ", ".join(
        #         [
        #             "%s: %.4f" % (name, value)
    ...
    def train(self):
        ...
        total_steps = int(len(train_loader) * self.hyper.epoch_num // 1)
        ...
        for epoch in range(self.hyper.epoch_num):
            ...
            if epoch % self.hyper.print_epoch == 0: # and epoch >= 2:
                ...
                logging.info("Dev, Epoch=%d: " % (epoch) + log)
                if new_score >= score:
                    score = new_score
                    best_epoch = epoch
                    self.save_model("best")
                # _, log_test = self.evaluation(test_loader)
                # logging.info("Test, Epoch=%d: " % (epoch) + log_test)
